...
export function InlineCell({
  ...
  return (
    ...
    >
      ...
      {onBgColorChange && (
        <div ref={colorPickerRef} className="absolute top-1 right-1 opacity-0 group-hover:opacity-100 transition-opacity z-10">
          <button
          ...
          {showColorPicker && (
            <div className="absolute right-0 top-6 bg-white border border-gray-200 rounded-lg shadow-xl p-2 flex flex-wrap gap-1 w-[116px] z-20">
              {BG_COLORS.map((c) => (
                <button
                  key={c.label}
                  title={c.label}
